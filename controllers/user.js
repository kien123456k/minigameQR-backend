...
module.exports = {
  register: (req, res, next) => {
    ...
    user.findOne({token: token}, function (err, student1) {
      if (!student1) {
        ...
        user.findOne({studentID: studentID.toUpperCase()}, (err, student2) => {
          if (student2) {
            const date = new Date();
            res.status(403).json({
              success: false,
        ...
        if (student1.studentID.toUpperCase() !== studentID) {
          const date = new Date();
          res.status(403).json({
            success: false,
          ...
          if (student1.name.toUpperCase() !== name.toUpperCase()) {
            const date = new Date();
            res.status(403).json({
              success: false,
  ...
  start: (req, res, next) => {
    ...
    user.findOne(
      {token: token, studentID: studentID.toUpperCase(), name: name},
    ...
    user.findOne(
      {token: token, name: name, studentID: studentID.toUpperCase()},
      (err, student) => {
        if (!student) {
          const date = new Date();
          res.status(400).json({
